...
export default function ProfilePage() {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (orgsRes.ok) {
        const orgsData = (await orgsRes.json()) as unknown[] | { items?: unknown[] };
        const items = Array.isArray(orgsData) ? orgsData : (orgsData.items ?? []);
        setOrgCount(items.length);
      }
    } catch {
    ...
    <div className="relative flex flex-col h-full bg-surface-sunken min-h-0">
      ...
      <header className="relative shrink-0 overflow-hidden border-b border-border">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 20% 20%, var(--color-accent-soft) 0%, transparent 45%), radial-gradient(circle at 80% 0%, var(--color-accent2-soft, var(--color-accent-soft)) 0%, transparent 40%)",
          }}
          aria-hidden
        />
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-border bg-gradient-to-br from-accent/5 to-accent2/5 p-5">
              ...
              <p className="text-[12px] text-muted leading-relaxed">
                Use a clear photo and your real name so teammates recognize you in comments,
                assignments, and activity feeds.
              </p>
            </section>
          </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[12px] text-muted truncate">Switch or create orgs</p>
                  </div>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[14px] font-semibold text-fg">New workspace</p>
                    <p className="text-[12px] text-muted truncate">Start a fresh project space</p>
                  </div>
                  <ChevronRight className="h-4 w-4 text-muted group-hover:text-accent transition-colors" />
                </Link>
              </div>
