CREATE TYPE "public"."notification_target_type" AS ENUM('task', 'comment', 'sprint', 'project', 'workspace', 'organization', 'invite', 'user', 'activity', 'none');--> statement-breakpoint
CREATE TYPE "public"."notification_type" AS ENUM('mention', 'task_assignment', 'task_comment', 'task_status', 'sprint_update', 'project_update', 'invite', 'activity', 'reminder', 'general');--> statement-breakpoint
CREATE TYPE "public"."org_role" AS ENUM('member', 'admin', 'owner');--> statement-breakpoint
CREATE TYPE "public"."workspace_role" AS ENUM('admin', 'project_manager', 'member');--> statement-breakpoint
...
CREATE TABLE "notifications" (
	...
	"target_type" "notification_target_type" DEFAULT 'none' NOT NULL,
	"target_id" uuid,
...
ALTER TABLE "notifications" ADD CONSTRAINT "notifications_origin_user_id_users_id_fk" FOREIGN KEY ("origin_user_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE cascade;--> statement-breakpoint
...
CREATE UNIQUE INDEX "tasks_project_ticket_number_uq" ON "tasks" USING btree ("project_id","ticket_number");--> statement-breakpoint
